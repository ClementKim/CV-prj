...
class ProposedMethod(nn.Module):
    def __init__(self, encoder, self_expert_pool, cross_expert_pool, aux):
...
def main():
    ...
    auxiliary_tokens = torch.randn(1, embed_dim)

    model = ProposedMethod(encoder, self_expert_pool, cross_expert_pool, auxiliary_tokens).to(device)
